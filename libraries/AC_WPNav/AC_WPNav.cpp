...
// @User: Standard
		...
		// @Increment: 10
		// @User: Standard
		AP_GROUPINFO("SPEED_DN", 3, AC_WPNav, _wp_speed_down_cms,
				WPNAV_WP_SPEED_DOWN),
		...
		// @Increment: 10
		// @User: Standard
		AP_GROUPINFO("ACCEL_Z", 6, AC_WPNav, _wp_accel_z_cmss,
...
// their values.
//
AC_WPNav::AC_WPNav(const AP_InertialNav &inav, const AP_AHRS_View &ahrs,
		AC_PosControl &pos_control, const AC_AttitudeControl &attitude_control) :
		_inav(inav), _ahrs(ahrs), _pos_control(pos_control), _attitude_control(
				attitude_control)
{
	...
	_wp_accel_cmss = MIN(_wp_accel_cmss,
			GRAVITY_MSS * 100.0f
					* tanf(ToRad(_attitude_control.lean_angle_max() * 0.01f)));
...
void AC_WPNav::set_speed_xy(float speed_cms)  //改变航点速度
{
...
bool AC_WPNav::set_wp_destination(const Location_Class &destination)
{
	...
	// convert destination location to vector 将目标位置转换为向量
...
bool AC_WPNav::set_wp_destination_NED(const Vector3f &destination_NED)
{
	...
	return set_wp_destination(
			Vector3f(destination_NED.x * 100.0f, destination_NED.y * 100.0f,
					-destination_NED.z * 100.0f), false);
}
...
bool AC_WPNav::set_wp_origin_and_destination(const Vector3f &origin,
		const Vector3f &destination, bool terrain_alt)
{
	...
	// initialise intermediate point to the origin 初始化中间点到原点
	_pos_control.set_pos_target(origin + Vector3f(0, 0, origin_terr_offset));
	_track_desired = 0;             // target is at beginning of track 目标在开始跟踪
	_flags.reached_destination = false;
	_flags.fast_waypoint = false;   // default waypoint back to slow
	_flags.slowing_down = false;    // target is not slowing down yet
	_flags.segment_type = SEGMENT_STRAIGHT; //线段类型为直线
	_flags.new_wp_destination = true; // flag new waypoint so we can freeze the pos controller's feed forward and smooth the transition 标记新的路径点，这样我们可以冻结pos控制器的前馈并平滑过渡
	_flags.wp_yaw_set = false;
	...
	float speed_along_track = curr_vel.x * _pos_delta_unit.x
			+ curr_vel.y * _pos_delta_unit.y + curr_vel.z * _pos_delta_unit.z;
	_limited_speed_xy_cms = constrain_float(speed_along_track, 0,
			_wp_speed_cms);
...
bool AC_WPNav::advance_wp_target_along_track(float dt)
{
	...
	float leash_z =
			track_error.z >= 0 ?
					_pos_control.get_leash_up_z() :
					_pos_control.get_leash_down_z();
	...
	float track_error_max_abs = MAX( //从车辆到轨迹上最近点的直线。它是对边
			_track_leash_length * track_error_z / leash_z,
			_track_leash_length * _track_error_xy
					/ _pos_control.get_leash_xy()); //_pos_control.get_leash_xy()为dt时间内允许移动的最大水平距离
	track_leash_slack = //从跑道上最近的点到目标点的线。它是邻边
			(track_leash_length_abs > track_error_max_abs) ?
					safe_sqrt(
							sq(_track_leash_length) - sq(track_error_max_abs)) :
					0; //偏差比最大限制距离还大，即飞机离航迹很远，这时飞机停止前进
	...
	} else
	{
		...
		_limited_speed_xy_cms = constrain_float(_limited_speed_xy_cms, 0.0f, //正常限幅
				_track_speed);
		...
		if (!_flags.fast_waypoint) //如果不是快速航点
		{
			...
			{
				_limited_speed_xy_cms = MIN(_limited_speed_xy_cms, //即将到达航点，正常减速
	...
	{//如果不是快速航点则期望“模长”要小于等于总“模长”
		_track_desired = constrain_float(_track_desired, 0, _track_length); //目标距离限幅
	} else
	{//如果是快速航点则期望“模长”要小于等于总“模长”+2m
		_track_desired = constrain_float(_track_desired, 0,
				_track_length + WPNAV_WP_FAST_OVERSHOOT_MAX);
	}
	...
	{ //检查是否到达终点
		if (_track_desired >= _track_length)
		{
			...
			} else
			{
				...
				Vector3f dist_to_dest = (curr_pos - Vector3f(0, 0, terr_offset))
						- _destination; //计算当前坐标点与目标坐标点的三维差值
	...
	{ //如果总路径长度大于等于2米
		...
		} else
		{
			...
			horiz_leash_xy.z = 0;
			if (horiz_leash_xy.length()
					> MIN(WPNAV_YAW_DIST_MIN,
							_pos_control.get_leash_xy()*WPNAV_YAW_LEASH_PCT_MIN))
			{
				set_yaw_cd(
						RadiansToCentiDegrees(
								atan2f(horiz_leash_xy.y, horiz_leash_xy.x))); //设置偏航对准目标点
			}
		}
	}
...
void AC_WPNav::calculate_wp_leash_length()
{
	...
	{
		_track_accel = MIN(_wp_accel_z_cmss / pos_delta_unit_z,
				_wp_accel_cmss / pos_delta_unit_xy);
		_track_speed = MIN(speed_z / pos_delta_unit_z,
				_wp_speed_cms / pos_delta_unit_xy);
		_track_leash_length = MIN(leash_z / pos_delta_unit_z, //dt时间内能移动的最大水平距离、最大垂直距离，取小值得到dt时间内能移动的最大值
				_pos_control.get_leash_xy() / pos_delta_unit_xy);
	}
...
bool AC_WPNav::set_spline_destination(const Location_Class &destination,
		bool stopped_at_start, spline_segment_end_type seg_end_type,
		Location_Class next_destination)
{
	...
	// set target as vector from EKF origin
	return set_spline_destination(dest_neu, dest_terr_alt, stopped_at_start,
			seg_end_type, next_dest_neu);
}
...
bool AC_WPNav::set_spline_destination(const Vector3f &destination,
		bool terrain_alt, bool stopped_at_start,
		spline_segment_end_type seg_end_type, const Vector3f &next_destination)
{
	Vector3f origin;
	...
	// set origin and destination
	return set_spline_origin_and_destination(origin, destination, terrain_alt,
			stopped_at_start, seg_end_type, next_destination);
}
...
bool AC_WPNav::set_spline_origin_and_destination(const Vector3f &origin,
		const Vector3f &destination, bool terrain_alt, bool stopped_at_start,
		spline_segment_end_type seg_end_type, const Vector3f &next_destination)
{
	...
	bool prev_segment_exists = (_flags.reached_destination
			&& ((AP_HAL::millis() - _wp_last_update) < 1000));
	...
	float vel_len = _spline_origin_vel.length()
			+ _spline_destination_vel.length();
	...
	if (vel_len > pos_len)
	{
		...
		update_spline_solution(origin, destination,
				_spline_origin_vel * vel_scaling,
				_spline_destination_vel * vel_scaling);
	} else
	{
		// update spline calculator
		update_spline_solution(origin, destination, _spline_origin_vel,
	...
	_track_length_xy = safe_sqrt(
			sq(_destination.x - _origin.x) + sq(_destination.y - _origin.y)); // horizontal track length (used to decide if we should update yaw)
...
void AC_WPNav::update_spline_solution(const Vector3f &origin,
		const Vector3f &dest, const Vector3f &origin_vel,
		const Vector3f &dest_vel)
{
	_hermite_spline_solution[0] = origin;
	_hermite_spline_solution[1] = origin_vel;
	_hermite_spline_solution[2] = -origin * 3.0f - origin_vel * 2.0f
			+ dest * 3.0f - dest_vel;
	_hermite_spline_solution[3] = origin * 2.0f + origin_vel - dest * 2.0f
			+ dest_vel;
}
...
bool AC_WPNav::advance_spline_target_along_track(float dt)
{
	if (!_flags.reached_destination)
	{
		...
		float track_leash_slack = MIN(
				_track_leash_length * (leash_z - track_error_z) / leash_z,
				_track_leash_length * (leash_xy - _track_error_xy) / leash_xy);
		...
		{
			_spline_vel_scaler = safe_sqrt(
					spline_dist_to_wp * 2.0f * _wp_accel_cmss);
		...
		_spline_vel_scaler = constrain_float(_spline_vel_scaler, 0.0f,
				vel_limit);
		...
		if (_track_length_xy >= WPNAV_YAW_DIST_MIN)
		{
			...
			} else
			{
								...
								_pos_control.get_leash_xy()*WPNAV_YAW_LEASH_PCT_MIN))
				{
					...
					set_yaw_cd(
							RadiansToCentiDegrees(
									atan2f(-track_error.y, -track_error.x)));
				}
			}
		}
...
void AC_WPNav::calc_spline_pos_vel(float spline_time, Vector3f &position,
		Vector3f &velocity)
{
...
bool AC_WPNav::get_vector_NEU(const Location_Class &loc, Vector3f &vec,
		bool &terrain_alt)
{
